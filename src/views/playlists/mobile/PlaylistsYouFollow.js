    ...
    async function getPlaylists (){
      try {
        if(props.selectedFollowedPlaylist){
          setSelectedPlaylist(props.selectedFollowedPlaylist);
          setLoadingPlaylists(false);
        }
        setPlaylists(props.playlistsYouFollow);
        setLoadingPlaylists(false);
      } catch(error) {
        ...
        <React.Fragment>
          {selectedPlaylist ?
            <React.Fragment>
              ...
              <PlaylistDetails selectedPlaylist={selectedPlaylist} followed={true}/>
            </React.Fragment>
          :
            <React.Fragment>
              <Grid item xs={12} style={{marginBottom:'16px'}} >
                <Reorder fontSize='large' style={{paddingRight:'16px', float:'left'}} />
                <Typography variant='h4' style={{float: 'left', paddingTop:'6px' }}>Playlists You Follow</Typography>
              </Grid>  
              <Grid container spacing={4} style={{ paddingRight:'16px', paddingLeft:'16px'}}>
                { playlists.map(p => (
